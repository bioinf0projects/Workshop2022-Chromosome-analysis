...
library("gridExtra")
# set the directory
setwd("~/Desktop/WS-repeat/Results") 
  
# Step 2
#upload satelite data
chr="Chr9"
satelite<-read.table("~/Desktop/WS-repeat/Results/XT_Chr9.fa.2.5.7.80.10.50.200.dat.nr.tab.map",header=FALSE)
satelite
map1<-setNames(satelite,c("chr","PosX","PosY","sat_span","sat_number","sat_type"))
# select the data corresponding to the chromosome
...
xlab <-"Xt Chr. 9 coordinate in bp"
ylab <-"Nb of satellites per 150 kbp"

#Step 3.1 to build the lines, we used the Line plot function

ggplot(map2)+geom_line(aes(y=sat_number,x=PosX,color=sat_type),size=1)+ylab("Nb of satellites per 150 kbp")+xlab("Xt Chr. 9 coordinate in bp")+ggtitle("satellite density on X.tropicalis chromosome 9")
...
plot_trf_dat<-ggplot(map2)+geom_line(aes(y= sat_number,x= PosX,color=sat_type),size=1)+ylab("Nb of satellites per 150 kbp")+xlab("Xt Chr. 9 coordinate in bp")+ggtitle("satellite density on X.tropicalis chromosome 9")
plot_trf_dat+facet_wrap(~sat_type)

# Step 3.3 Best pick
#a- The base plot is made to see the area under each line
ggplot(map2,aes(y= sat_number,x= PosX,fill=sat_type))+geom_area()

#to change the name, we define this plot as being plot0
plot0<-ggplot(map2,aes(y= sat_number,x= PosX,fill=sat_type))+geom_area()

#b- We change the appearance of the drawing to increase readability. 
#This makes the plot look more defined
ggplot(map2,aes(y= sat_number,x= PosX,fill=sat_type))+geom_area(colour="black",size=0.2)
#c- to differentiate from plot0, we define this plot as being plot1
plot1<-ggplot(map2,aes(y= sat_number,x= PosX,fill=sat_type))+geom_area(colour="black",size=0.2)
#and then we have to check plot1
plot1

#d- We add names for the axis x and y and changed the title
plot1+ylab("Nb of satellites per 150 kbp")+xlab("Chr 9 XENLOPUS coordinate in bp")+ggtitle("SATELLITE DENSITY ON X. tropicalis CHROMOSOME 9")
plot2<-plot1+ylab("Nb of satellites per 150 kbp")+xlab("Chr 9 XENOPUS coordinate in bp")+ggtitle("SATELLITE DENSITY ON X. tropicalis CHROMOSOME 9")
...
plot4<-plot3+theme(axis.text=element_text(face="bold",size=12),axis.title=element_text(face="bold",size=14),title=element_text(face="bold",size=16),plot.title = element_text(hjust = 0.5))
